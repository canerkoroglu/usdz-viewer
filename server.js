 ...
 *   - GET /api/models        -> metadata for supported model files in DATA_DIR
...
const ALLOWED_EXT = new Set(['usd', 'usda', 'usdc', 'usdz']);
...
function resolveModelPath(rawName) {
  ...
  const resolved = path.resolve(DATA_DIR, base);

  // The resolved file must live directly inside DATA_DIR.
  ...
  try {
    ...
    try {
      ...
      if (err.code === 'ENOENT') {
        ...
        return res.status(200).json({ models: [], dataDir: DATA_DIR, warning: 'data directory not found' });
      }
      throw err;
    }
    ...
    for (const entry of entries) {
      ...
      if (!ALLOWED_EXT.has(extOf(name))) continue;
...
app.get('/models/:filename', (req, res) => {
  const resolved = resolveModelPath(req.params.filename);
  if (!resolved) {
  ...
  fs.stat(resolved, (statErr, stat) => {
    ...
    res.sendFile(resolved, { dotfiles: 'deny' }, (sendErr) => {
      if (sendErr && !res.headersSent) {
        console.error('[models] send failed:', sendErr);
        res.status(500).json({ error: 'Failed to send model' });
      } else if (sendErr) {
